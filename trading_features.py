...
def add_indicators(df):
    ...
    df['RSI_70'] = ta.momentum.RSIIndicator(close=df['Close'], window=70).rsi()
...
def build_observation(df, current_step, balance, btc_held, last_trade_profits):
    ...
    # Get last 5 RSI-30 and RSI-70 values (pad with 0.0 if not enough)
    rsi_30_vals = [
        get_scalar(df.iloc[max(0, current_step - i)]['RSI_30'])
        for i in range(5)
    ]
    rsi_70_vals = [
        get_scalar(df.iloc[max(0, current_step - i)]['RSI_70'])
        for i in range(5)
    ]
